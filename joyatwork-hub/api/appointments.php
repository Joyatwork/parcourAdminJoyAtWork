...
try {
    ...
    // Debug log
    error_log("Appointments API - Method: " . $_SERVER['REQUEST_METHOD']);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if ($count['total'] == 0) {
            ...
            error_log("Appointments API - Fetching real data from database");
            $sql = "SELECT * FROM appointments ORDER BY scheduled_at DESC LIMIT 100";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            $appointments = $stmt->fetchAll(PDO::FETCH_ASSOC);
            error_log("Appointments API - Found " . count($appointments) . " appointments");
