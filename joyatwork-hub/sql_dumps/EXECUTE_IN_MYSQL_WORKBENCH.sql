...
-- 2. Insertion des 8 entreprises d'exemple
...
SELECT 'Table companies créée avec succès !' as status;
SELECT COUNT(*) as 'Nombre d\'entreprises' FROM companies;

-- 4. Affichage des premières entreprises
SELECT id, name, sector, employees, contract_value, status FROM companies LIMIT 5;

-- 5. Statistiques
SELECT 
    COUNT(*) as total_entreprises,
    SUM(employees) as total_employes,
    ROUND(SUM(contract_value), 0) as chiffre_affaires_total,
    ROUND(AVG(contract_value), 0) as contrat_moyen
FROM companies;
